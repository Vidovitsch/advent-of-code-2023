      ...
      if schematic.is_gear():
        ...
        for prod in product((-1, 0, 1), repeat=2):
          ...
          if schematic.is_on_digit():
            gear_ratio_count += 1
            gear_ratio *= schematic.find_number()
...
if __name__ == '__main__':
  (Puzzle('Day 3: Gear Ratios (part 2)', CURRENT / 'input.txt')
    .add_test({ 'input_path': CURRENT / 'test_1.txt', 'expected_result': 467835 })
    .add_test({ 'input_path': CURRENT / 'input.txt', 'expected_result': 75312571 })
    .solve(solve))
